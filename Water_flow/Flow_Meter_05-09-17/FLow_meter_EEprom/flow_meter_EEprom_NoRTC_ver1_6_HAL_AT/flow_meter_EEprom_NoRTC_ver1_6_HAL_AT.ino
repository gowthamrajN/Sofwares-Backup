...
void loop() 
{
  ...
  showLCD(Sum,units);
  CurrentTime=millis();
  if(CurrentTime-PreviousTime>=3600000)             //43200000 for half day  //86400000 for one day //21600000 for 6hrs  //3600000 for 1hr  //10800000 for 3 hors//60000 for 1 min //600000 for 10 min
  {
    falconClient(jsn);
    PreviousTime=millis();
  }
...
{ 
   ...
   litres_long=FlowFrequency;
    ...
    Sum=Sum+litres_long+1;                                                           //just for check
    //Sum=Sum+litres_long;
    Litre_Str=String(litres_long);
